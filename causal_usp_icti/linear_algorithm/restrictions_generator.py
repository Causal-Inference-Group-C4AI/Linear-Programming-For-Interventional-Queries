...
class RestrictionsGenerator:
    """
    Given an intervention and a graph, this class finds a set of restrictions that can be used to build
    a linear objective function.
    """
    
    def __init__(self, graph: Graph, intervention: int, target: int | str, interventionValue: int, targetValue: int,
                dataFrame, empiricalProbabilitiesVariables: list[int], mechanismVariables: list[int],  
    ...
    def generate_restrictions(self):
        ...
        conditionalProbabilities: dict[int, list[int]] = {} # If V|A,B,C in this array then it implies P(V|A,B,C) in the objective function
        debugOrder: list[int] = []
    ...
    def build_objective_function(self):
        ...
        # (1) Remove c-component variables that do not appear in the objective function
        endogenousNodes = ( set(cComponentEndogenous) & set(self.debugOrder)) | { self.intervention }
                
        _, _, mechanisms = MechanismGenerator.mechanisms_generator(latentNode=interventionLatentParent, 
        ...
        """        
        objFunctionCoefficients: list[int] = []       
        ...
        for mechanism in mechanisms:            
            ...
            for inputCase in inputCases:
                ...
                variablesValues: dict[int, int] = {self.intervention: self.interventionValue, self.target: self.targetValue}
                ...
                for variable in summandNodes:
                    ...
                    elif variable in self.mechanismVariables: # Case 2: terminate with coeff 0 if the decision function is 0. Do nothing otherwise
                        print("Case 2")
                        mechanismKey: str = ""
                        for nodeIndex, node in enumerate(self.graph.graphNodes):
                            if not node.isLatent and (variable in node.children):                                
                                mechanismKey += f"{nodeIndex}={variablesValues[nodeIndex]},"
    ...
    def test():
        ...
        rg = RestrictionsGenerator(graph=graph, intervention=graph.labelToIndex["X"], interventionValue=0,target=graph.labelToIndex["Y"],
                                    targetValue=1,empiricalProbabilitiesVariables=[], mechanismVariables=[], conditionalProbabilitiesVariables={},
                                    debugOrder=[], dataFrame=df)
        ...
        for node in rg.debugOrder:            
            if node in rg.empiricalProbabilitiesVariables:
                ...
                wset: str = ""
                for condVar in rg.conditionalProbabilities[node]:
        ...
        ----------
        """
        objCoefficients = rg.build_objective_function()
