...
class GeneralProblem:
    def __init__(self, dataFrame, empiricalProbabilities, parametric_columns):
        self.intervention_label = "X"
        self.intervention_value = 1
        self.target_label = "Y"
        self.target_value = 1
    ...
    def _generate_patterns(self):
        ...
        while True:
            ...
            mechanisms = None
            free_variables = []
            variableLabelToBitIndex = dict[str, int]
            '''
                "D":1,
                "E":2,
                ...
            '''
            objCoeff: float = self._getCoefficient(latentRealization, free_variables=free_variables, probabilities=probabilities, mechanisms=mechanisms, variableLabelToBitIndex=variableLabelToBitIndex)
    ...
    def _getCoefficient(self, latentRealization: list[int], free_variables: list[str], probabilities: list[tuple[list[str], list[str]]], mechanisms: list[(str, int, dict[str, int])], variableLabelToBitIndex: dict[str, int]):
        coeffValue: float = 0
        for realizationCase in range(2**len(free_variables)):
            binary = format(realizationCase, f'0{len(free_variables)-1}b')
            currCoeff: float = 1
            for prob in probabilities:
                conditionedRealization = {}
                for conditionedVar in prob[0]:
                    if conditionedVar == self.intervention_label:
                        conditionedValue = self.intervention_value
                    elif conditionedVar == self.target_label:
                        conditionedValue = self.target_value
                    else:
                        conditionedValue = int(binary[variableLabelToBitIndex[conditionedVar]])
                    conditionedRealization[conditionedVar] = conditionedValue

                conditioningRealization = {}
                for conditioningVar in prob[1]:
                    if conditioningVar == self.intervention_label:
                        conditioningValue = self.intervention_value
                    else:
                        conditioningValue = int(binary[variableLabelToBitIndex[conditioningVar]])
                    conditioningRealization[conditioningVar] = conditioningValue

                if bool(conditioningRealization):
                    probCoeff *= ph.find_conditional_probability2(dataFrame=self.dataFrame, targetRealization=conditionedRealization, conditionRealization=conditioningRealization)
                else:
                    probCoeff = ph.find_probability2(dataFrame=self.dataFrame, realizationDict=conditionedRealization)

                currCoeff *= probCoeff

                if currCoeff == 0:
                    break
            coeffValue += currCoeff
            '''
            y_ = get_y_form_mechanisms()
            if y_ != y:
                coeffValue = 0
            '''
...
def main():
    ...
    itauProblem = GeneralProblem(dataFrame=df, empiricalProbabilities=empiricalProbabilities, parametric_columns=parametric_columns)
    itauProblem.solve()
