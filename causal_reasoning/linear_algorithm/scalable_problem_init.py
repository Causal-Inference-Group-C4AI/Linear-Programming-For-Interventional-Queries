...
class InitScalable:
    ...
    def defineGammaUAuxiliaryVariables(M: int, N: int, df, XValue: int, targetValue: int, DBG = False):
        ...
        for betaVarIndex, realizationCase in enumerate(list(product([0, 1], repeat= M + N))):            
            ...
            objCoeff: float = 1.0
            objCoeff *= ph.find_conditional_probability2(dataFrame=df, targetRealization= { "Y": targetValue },
            ...
            objCoeff *= ph.find_conditional_probability2(dataFrame=df, targetRealization = targetRealization,
...
def testBetaVars():
    ...
    for realizationCase in list(product([0, 1], repeat=4)):
        prob = ph.find_conditional_probability2(dataFrame=df,
